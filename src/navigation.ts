...
export type NavigationItem = { to: string; label: string; ownerOnly?: boolean; gradeAdmin?: boolean; homeroom?: boolean; homeroomOrOwner?: boolean };
...
export function visibleNavigation(user: AppUser | null, assignments: StaffAssignment[], scope: { academicYearId: string; gradeId: string | null } | null, classes: ClassRoom[] = []) {
  ...
  return navigationItems.filter((item) => !item.ownerOnly || owner).filter((item) => !item.gradeAdmin || owner || admin).filter((item) => !item.homeroom || homeroom).filter((item) => !item.homeroomOrOwner || owner || homeroom);
}
